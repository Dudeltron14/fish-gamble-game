...
func _check() -> void:
	...
	var audited_card: Dictionary = server._draw_card("Tester", "deal", 7)
	assert(server._shoe_audit == [{"actor": "Tester", "action": "deal", "card": audited_card, "hand_id": 7}])
	server._record_audit("Tester", "stand", 7)
	assert(server._shoe_audit.back() == {"actor": "Tester", "action": "stand", "hand_id": 7})
	server.free()
	quit()
