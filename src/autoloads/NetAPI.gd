...
signal fishing_start(ok: bool, fish_id: String, difficulty: float, cast_speed: float, line_strength: float)
...
@rpc("authority", "call_local", "reliable")
func notify_fishing_start(ok: bool, fish_id: String, difficulty: float, cast_speed: float, line_strength: float) -> void:
	if multiplayer.is_server() and not GameManager.is_hosting: return
	fishing_start.emit(ok, fish_id, difficulty, cast_speed, line_strength)
