...
func _show_ledger(ledger: Dictionary) -> void:
	GameManager.set_coins(int(ledger.get("coins", GameManager.current_coins)))
	var reset_at := int(ledger.get("reset_at", 0))
	var reset_text := ""
	if reset_at > Time.get_unix_time_from_system():
		var seconds_left := reset_at - int(Time.get_unix_time_from_system())
		reset_text = "  New ledger in %dh %02dm." % [seconds_left / 3600, (seconds_left % 3600) / 60]
	%Info.text = "%d free rerolls left today. More rerolls cost %d gold.  Purse: %d gold%s" % [int(ledger.get("free_rerolls_left", 0)), int(ledger.get("reroll_cost", 25)), int(ledger.get("coins", 0)), reset_text]
	...
	for quest: Dictionary in ledger.get("quests", []):
		...
		row.add_child(label)
		var reroll := Button.new(); reroll.text = "Reroll"; reroll.disabled = done or int(quest.get("claimed", 0)) != 0; reroll.pressed.connect(func(): NetAPI.rpc_id(1, "c2s_daily_quest_reroll", int(quest.slot))); row.add_child(reroll)
